...
namespace Yarışma.Areas.Management.Controllers
{
    ...
    public class ContestantTableController : Controller
    {
        ...
        public IActionResult GetContestants(int page = 1, int pageSize = 10)
        {
            ...
            return PartialView("_ContestantTablePartial", vm);
        }
        public IActionResult ExportToExcel()
        {
            // Veritabanından yarışmacıları ve ilişkili projeleri alın
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                foreach (var contestant in contestants)
                {
                    ...
                    var sanayiciHakemi = judges.FirstOrDefault(j => j.JudgeCategory.Name == "Sanayici" && db.ContestantJudges.Any(cj => cj.ContestantId == contestant.Id && cj.JudgeId == j.Id));
                    var akademisyenHakemi = judges.FirstOrDefault(j => j.JudgeCategory.Name == "Akademisyen" && db.ContestantJudges.Any(cj => cj.ContestantId == contestant.Id && cj.JudgeId == j.Id));
